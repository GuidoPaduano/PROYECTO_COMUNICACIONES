...
interface CachePromise<T> {
  key: string
  promise: Promise<T>
}

export interface FetchPublicJsonOptions extends RequestInit {
  retries?: number
  retryDelayMs?: number
}

export interface AuthFetchOptions extends RequestInit {
  skipProfileCache?: boolean
  skipProfileApiCache?: boolean
}

export interface GetSessionProfileOptions {
  force?: boolean
  maxAgeMs?: number
  profile?: object
}
...
export async function fetchPublicJson(path: string, options: FetchPublicJsonOptions = {}): Promise<unknown> {
  const {
    retries = 1,
    retryDelayMs = 300,
    ...fetchOptions
...
let sessionProfileCache: CacheEntry<unknown> | null = null
let sessionProfilePromise: CachePromise<unknown> | null = null
let profileApiCache: CacheEntry<unknown> | null = null
let profileApiPromise: CachePromise<unknown> | null = null
...
function normalizeHostname(rawHost: unknown): string {
...
export function getSchoolSlugFromHost(rawHost = ""): string {
...
function getRelativeLocation(): string {
  try {
...
export function sanitizePostLoginPath(rawPath = ""): string {
...
function dispatchSessionContext(context: SessionContext | null): void {
  try {
...
export function normalizeSchool(rawSchool: unknown): School | null {
  if (!rawSchool || typeof rawSchool !== "object") return null

  const s = rawSchool as Record<string, unknown>
  const id = (s.id ?? s.school_id ?? null) as number | string | null
  const name = String(s.name || s.school_name || "").trim()
  const shortName = String(s.short_name || s.school_short_name || "").trim()
  const slug = String(s.slug || s.school_slug || "").trim()
  const logoUrl = String(s.logo_url || s.school_logo_url || "").trim() || DEFAULT_SCHOOL_LOGO_URL
  const isActive = s.is_active !== false
  const primaryColor = normalizeHexColor(
    s.primary_color || s.school_primary_color,
    DEFAULT_SCHOOL_PRIMARY_COLOR
  )
  const accentColor = normalizeHexColor(
    s.accent_color || s.school_accent_color,
...
function buildPublicBranding(school: unknown, fallback: PublicBranding = DEFAULT_PUBLIC_BRANDING): PublicBranding {
  const normalizedFallback: PublicBranding = {
...
function getSchoolParamFromWindow(): string {
  try {
...
export function getHostSchoolSlugFromWindow(): string {
  try {
    if (typeof window === "undefined") return ""
...
export function getRequestedSchoolIdentifierFromWindow(): string {
  return getSchoolParamFromWindow() || getHostSchoolSlugFromWindow() || ""
}

export function getLastSessionSchool(): School | null {
  try {
...
function rememberLastSessionSchool(rawSchool: unknown): void {
...
export function rememberSchoolSelection(rawSchool: unknown): void {
  rememberLastSessionSchool(rawSchool)
}

export function getLastSchoolLoginHref(): string {
  const school = getLastSessionSchool()
  return school ? buildSchoolLoginHref(school) : ""
}

export function buildSchoolLoginHref(school: unknown): string {
  const normalizedSchool = normalizeSchool(school)
...
export function usePublicSchoolBranding(options: { fallback?: PublicBranding; clearSession?: boolean } = {}): PublicBranding {
  const {
    fallback = DEFAULT_PUBLIC_BRANDING,
    clearSession = false,
  } = options
  const [branding, setBranding] = useState<PublicBranding>(() => buildPublicBranding(null, fallback))
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const data = await fetchPublicJson(path, { retries: 1, retryDelayMs: 300 }) as Record<string, unknown> | null
        if (alive) {
...
function normalizeSchoolList(rawSchools: unknown): School[] {
...
function normalizeGroups(rawGroups: unknown): string[] {
  ...
  return rawGroups
    .map((group) => (typeof group === "string" ? group : (group as Record<string, string>)?.name || (group as Record<string, string>)?.nombre || ""))
    .filter(Boolean)
}

export function buildSessionContext(payload: Record<string, unknown> = {}): SessionContext {
  const initialSchool = normalizeSchool(payload?.school || (payload?.user as Record<string, unknown>)?.school || null)
  const availableSchools = normalizeSchoolList(
    payload?.available_schools || payload?.availableSchools || (payload?.user as Record<string, unknown>)?.available_schools || []
  )
  const groups = normalizeGroups(
    payload?.groups || (payload?.user as Record<string, unknown>)?.groups || []
  )
  const username = String(payload?.username || (payload?.user as Record<string, unknown>)?.username || "").trim()
  const fullName = String(
    payload?.full_name ||
      (payload?.user as Record<string, unknown>)?.full_name ||
      [payload?.first_name, payload?.last_name].filter(Boolean).join(" ") ||
      [(payload?.user as Record<string, unknown>)?.first_name, (payload?.user as Record<string, unknown>)?.last_name].filter(Boolean).join(" ")
  ).trim()
  ...
  return {
    ...
    username,
    groups,
    role: String(payload?.rol || (payload?.user as Record<string, unknown>)?.rol || "").trim(),
    isSuperuser: !!payload?.is_superuser || !!(payload?.user as Record<string, unknown>)?.is_superuser,
    school,
    availableSchools,
  }
}
...
export function setSessionContext(context: SessionContext | null): SessionContext | null {
  try {
...
export function syncSessionContext(payload: Record<string, unknown> = {}): SessionContext | null {
  const previous = getSessionContext() || ({} as Partial<SessionContext>)
...
function schoolMatchesIdentifier(school: School, rawValue: unknown): boolean {
...
export function selectSessionSchool(rawValue: unknown): SessionContext | null {
...
function getSessionProfileCacheKey(): string {
  const context = getSessionContext()
...
function getCachedSessionProfile(): unknown | null {
...
export function getCachedSessionProfileData(): unknown | null {
  return getCachedSessionProfile()
}

function buildCachedJsonResponse(data: unknown): Response {
...
export function invalidateSessionProfileCache(): void {
...
export async function getSessionProfile(options: GetSessionProfileOptions = {}): Promise<unknown> {
...
function getProfileApiCacheKey(): string {
  const context = getSessionContext()
  const previewRole = getPreviewRole()
  const schoolRef =
    String(context?.school?.slug || context?.school?.id || getRequestedSchoolIdentifierFromWindow() || "").trim()
...
export function getCachedProfileApi(): unknown | null {
...
export async function getProfileApi(options: GetSessionProfileOptions = {}): Promise<unknown> {
  const force = options?.force === true
  const maxAgeMs =
    Number.isFinite(Number(options?.maxAgeMs)) && Number(options?.maxAgeMs) > 0
      ? Number(options.maxAgeMs)
...
export function getSessionAlumnoRouteId(profile: unknown): string | null {
  if (!profile || typeof profile !== "object") return null

  const p = profile as Record<string, unknown>
  const alumno = p?.alumno as Record<string, unknown> | undefined
  const pk = alumno?.id ?? alumno?.pk ?? null
  if (pk != null && String(pk).trim() !== "") return String(pk).trim()

  const legajo = alumno?.id_alumno ?? null
  if (legajo != null && String(legajo).trim() !== "") return String(legajo).trim()

  const username = String(p?.username ?? (p?.user as Record<string, unknown>)?.username ?? "").trim()
  if (username) return username

  return null
}

export async function resolveSessionAlumnoRouteId(options: GetSessionProfileOptions & { profile?: unknown } = {}): Promise<string | null> {
...
export function useSessionContext(): SessionContext | null {
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      setContext((event as CustomEvent<SessionContext | null>)?.detail ?? getSessionContext())
    }
    if (typeof window !== "undefined") {
...
function getLogoutRedirectHref(): string {
  try {
    const context = getSessionContext()
...
export function useRolePreview(): [string, (role: string) => void] {
  const [preview, setPreview] = useState<string>(() => getPreviewRole())

  useEffect(() => {
    const handler = (event: Event) => setPreview((event as CustomEvent<string>)?.detail ?? getPreviewRole())
    if (typeof window !== "undefined") {
  ...
  return [preview, (role: string) => {
    setPreviewRole(role)
    setPreview(role)
  }]
}

export function getEffectiveGroups(me: unknown): string[] {
...
function isProbablyLoginHtml(res: Response): boolean {
  try {
...
function forceRelogin(reason = "No autenticado"): never {
  clearTokens()
  if (typeof window !== "undefined") {
...
function shouldCaptureSessionContext(path: string): boolean {
  const normalized = normalizeApiPath(path).replace(/^\/+/, "").replace(/\/+$/, "")
  return normalized === "auth/whoami" || normalized === "perfil_api"
}

function captureSessionContext(path: string, res: Response): void {
...
function applySchoolHeader(headers: Headers): void {
...
export async function authFetch(path: string, opts: AuthFetchOptions = {}): Promise<Response> {
  ...
  if (method === "GET" && !opts?.skipProfileCache && normalizedPath === "auth/whoami") {
    ...
    if (cachedProfile) {
      syncSessionContext(cachedProfile as Record<string, unknown>)
...
export function useAuthGuard(options: { enabled?: boolean } = {}): void {
