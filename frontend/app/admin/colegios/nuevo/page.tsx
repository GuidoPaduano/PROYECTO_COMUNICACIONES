...
function flattenErrors(errors: Record<string, unknown>) {
  if (!errors || typeof errors !== "object") return []
  const lines = []
  for (const [field, val] of Object.entries(errors)) {
    const items = Array.isArray(val) ? (val as unknown[]) : [val]
...
export default function NuevoColegioPage() {
  ...
  const [created, setCreated] = useState<{ school?: { name?: string; slug?: string }; seeded_courses?: number; [key: string]: unknown } | null>(null)
