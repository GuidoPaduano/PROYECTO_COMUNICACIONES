...
export default function MisSancionesAlias() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        if (alumnoId) {
          router.replace(
            `/alumnos/${encodeURIComponent(alumnoId)}?from=mis-sanciones&tab=sanciones`
          )
        } else {
  ...
  return (
    <div className="flex items-center justify-center">
      <div className="surface-card surface-card-pad text-center text-gray-700 text-sm">
        Redirigiendo a <span className="font-semibold">Mis sanciones...</span>
      </div>
    </div>
  )
}
